...
export async function searchNotes(req, res) {
  try {
    const searchNote = await noteSchema.findOne({
      userId: req.idUser,
      _id: req.params["id"],
    });
...
export async function searchByName(req, res) {
  try {
    ...
    const foundNote = await noteSchema.find({
      title: titleToSearch,
      userId: req.idUser,
    });
    if (foundNote.length === 0)
      throw new Error(`No data found with ${titleToSearch}`);
    res.status(200).json({
      data: foundNote,
      message: "Data found",
      status: 200,
...
export async function showUpdatedData(req, res) {
  try {
    ...
    const sortedData = data
      .filter((ele) => ele.createdAt < ele.updatedAt)
...
export async function showSoftDeleteTrue(req, res) {
  try {
    const trueData = await noteSchema.find({
      userId: req.idUser,
      isSoftDelete: true,
    });
...
export async function deleteMultiple(req, res) {
  try {
    const arrayData = req.body.ids;
    const deleteNotes = await noteSchema.deleteMany({
      _id: { $in: arrayData },
      userId: req.idUser,
    });
    res.status(200).json({
